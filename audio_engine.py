...
@lru_cache(maxsize=1)
def _wave_class():
    """Load and return the mutagen WAVE class with a helpful error if missing."""
    spec = importlib.util.find_spec("mutagen.wave")
    if spec is None or spec.loader is None:
        raise RuntimeError(
            "The 'mutagen' package is required for metadata stripping. Install it via `pip install mutagen`."
        )
